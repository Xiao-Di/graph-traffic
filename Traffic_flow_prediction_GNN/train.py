...
def main():
    ...
    for epoch in range(Epoch):
        ...
        for data in train_loader:
            model.zero_grad()
            # predict_value = model(data, device).to(
            # torch.device("cuda"))  # [0, 1] -> recover
            predict_value = model(data, device).to(device)  # [0, 1] -> recover
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            # predict_value = model(data, device).to(
            # torch.device("cuda"))  # [B, N, 1, D]
            predict_value = model(data, device).to(device)  # [B, N, 1, D]
            if num == 0:
    ...
    plt.ylabel("traffic flow")
    plt.plot(
        test_data.recover_data(test_data.flow_norm[0], test_data.flow_norm[1], all_y_true.cpu(
        ).numpy())[:24 * 12, node_id, 0, 0],
        label='Ground Truth')
    plt.plot(
