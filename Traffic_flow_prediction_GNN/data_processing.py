...
class LoadData(Dataset):
    def __init__(self, data_path, num_nodes, divide_days, time_interval, history_length, train_mode):
        ...
        self.train_days = divide_days[0]  # 45 PEMS04一共59天
        self.test_days = divide_days[1]  # 14
        self.history_length = history_length  # 6
        self.time_interval = time_interval  # 5 min

        self.one_day_length = int(24 * 60 / self.time_interval) # 分钟

        self.graph = get_adjacent_matrix(distance_file=data_path[0], num_nodes=num_nodes) # graph_type="distance" ? 
    ...
    def normalize_base(data, norm_dim):
        ...
        :return:
            ...
            min_data: np.array
        """
        max_data = np.max(data, norm_dim, keepdims=True)  # [N, T, D] , norm_dim=1, [N, 1, D] # axis=1 每一列比较
...
def get_adjacent_matrix(distance_file: str, num_nodes: int, id_file: str = None, graph_type="connect") -> np.array:
    """
    :param distance_file: str, 用于保存节点之间距离的文件 .csv
